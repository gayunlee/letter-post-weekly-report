...
## 3분류 전환 + 프롬프트 최적화 (2026-03-12)

### 3분류 확정
5분류 → 4분류 → **3분류**: 대응 필요 / 콘텐츠·투자 / 노이즈
- "1:1 처리 테스트": 담당자가 1:1 회신/시스템 처리를 해야 하는가?
- 3축 체계: topic(라우팅) + tags(하위 분류) + sentiment(감정)
- 제안/피드백 → 콘텐츠·투자 + 태그(콘텐츠 제안/서비스 피드백/운영 의견)

### 프롬프트 벤치마크 결과 (R5, Haiku 4.5)
- Golden set: v8, 197건 (대응 66, 콘텐츠 82, 노이즈 49)
- **93.9%** (185/197)
- Per-class F1: 대응 0.947, 콘텐츠 0.927, 노이즈 0.950
- 여정: 78.9% → 84.1% → 87.2% → 89.8% → 92.4% → 93.9%

### 학습 데이터 (API 비용 $0)
기존 v3c 라벨을 기계적 3분류 매핑:
- 운영 피드백 → 대응 필요 / 서비스 피드백 → 대응 필요 (기능 제안 72건은 콘텐츠·투자)
- 콘텐츠·투자 → 콘텐츠·투자 / 일상·감사 → 노이즈 / 기타 → 노이즈
- 경계 케이스 수동 검수 완료
- 최종: 15,967건 (콘텐츠 75.2%, 대응 13.3%, 노이즈 11.5%)

### 파인튜닝 모델: Gemma (LoRA, 로컬)
- KcBERT 기각: 맥락 파악 부족 (110M 인코더)
- Qwen 기각: 핵심 기술 리더 사직 → 프로젝트 지속성 리스크
- Gemma 선정: Google DeepMind, 오픈소스, 시리즈 지속
- 환경: Mac 48GB MPS, LoRA

### 점진적 검증 전략
1. Zero-shot 테스트 (~5분) → 모델 기본 능력 확인
2. 소량 FT 1,000건 (~15분) → FT 효과 확인
3. 전량 FT 15,967건 (~30분) → 목표 93%+ 달성 확인
