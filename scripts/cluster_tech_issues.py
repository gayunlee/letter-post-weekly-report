...
def _load_domain_rules():
    rules_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'config', 'domain_rules.md')
    if os.path.exists(rules_path):
        with open(rules_path) as f:
            return f.read()
    return ""

DESCRIBE_SYSTEM_PROMPT = f"""이 텍스트에서 사용자가 말하고 있는 내용을 한 문장으로 요약하세요.

## 핵심 규칙
- **형식: "[서비스명] — 구체적 증상"** (서비스명을 반드시 맨 앞에)
  - 서비스명: 어스플러스 앱 / 어스캠퍼스 / 기업분석도감 / 줌 라이브 / 결제시스템 / 커뮤니티 게시판 등
  - 서비스명을 특정할 수 없으면 원문에서 언급된 화면/기능명을 사용
- 원문에 보이는 사실만 적을 것
- 평가/판단/의견/인과추정 금지
- "~로 보입니다" 같은 코멘트 금지
- 100자 이내, 한 문장만 출력
...
## 예시
- "어스플러스 앱 — 푸시 알림 설정 ON인데 새 글 알림 미수신"
- "어스캠퍼스 — 봄학기 결제 완료했으나 나의스터디에 강의 미표시"
- "기업분석도감 — 모바일 열람 시 글씨 확대(핀치줌) 미작동"
- "줌 라이브 — 특강 참여하기 버튼 클릭해도 화면 전환 없음"
- "결제시스템 — 가상계좌 입금 후 미등록 상태 표시"
- "커뮤니티 게시판 — 댓글 기능이 없어서 소통이 안 된다는 의견"
- "구독 — 자동결제가 되는 줄 몰랐다는 사용자의 환불 요청"
- "운영정책 — 멤버십 가입 조건에 대한 사용자 건의"
- "운영진 공지 — 앱 업데이트 관련 안내"
