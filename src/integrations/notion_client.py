...
class NotionReportClient:
    ...
    def _markdown_to_blocks(self, markdown: str) -> List[Dict[str, Any]]:
        ...
        while i < len(lines):
            ...
            # (총합: → 앞에 빈 줄 추가 (핵심 이슈와 구분)
            if line.startswith('(총합:'):
                if blocks and blocks[-1].get("type") == "paragraph" and blocks[-1]["paragraph"]["rich_text"]:
    ...
    def _create_classification_guide_toggle(self) -> List[Dict[str, Any]]:
        ...
        # 분류 기준 테이블 데이터
        classification_data = [
            ["카테고리", "세부 분류 (subtag)", "설명"],
            ["피드백", "환불·해지 / 결제·구독 / 접속·오류 / 기능요청 / UX개선 / 운영정책 / 이용문의", "운영팀·제품팀 확인 필요"],
            ["마스터 반응", "칭찬·감사 / 비판·불신 / 건강·안부 / 학습·성장 / 분석평가", "마스터에 대한 이용자 반응"],
            ["시장·투자", "종목분석 / 포트폴리오 / 시장전망 / 매매보고 / 투자심리 / 전략질문", "투자·시장 관련 논의"],
            ["일상", "-", "인사, 안부, 잡담 (액션 불필요)"],
        ]
